...
def make_Iridium_debris():
    ...
    for i in range(0 , len(norad_list)-1):
        norad = norad_list[i]
        inclination = np.deg2rad(inclination_list[i])
        raan = np.deg2rad(raan_list[i])
        eccentricity = eccentricity_list[i]
        arg_perigee = np.deg2rad(arg_perigee_list[i])
        mean_anomaly = np.deg2rad(mean_anomaly_list[i])
